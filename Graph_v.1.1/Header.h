...
Graph<int> makeGraph()
{
	...
	cout << "|-----------------------------------------|" << endl;
	cout << "|1.Ручной         |2.Автоматический       |" << endl;
	cout << " -----------------------------------------" << endl;
	cin >> mode;
	...
	switch (mode) {
		...
		case 2:
			...
			*/
			...
			sourceVertPtr = &sourceVertex; // запоминаем адрес исходной вершины
			targetVertPtr = &targetVertex; // запоминаем адрес конечной вершины (до которой будет идти ребро от исходной вершины)
			graph.InsertEdge(*sourceVertPtr, *targetVertPtr, edgeWeight); // вставка ребра весом edgeWeight между исходной и конечной вершинами
